<div>
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            Per page:
            <select wire:model="perPage" class="form-select w-full sm:w-1/6">
                @foreach($paginationOptions as $value)
                    <option value="{{ $value }}">{{ $value }}</option>
                @endforeach
            </select>

            @can('course_detail_delete')
                <button class="btn btn-rose ml-3 disabled:opacity-50 disabled:cursor-not-allowed" type="button" wire:click="confirm('deleteSelected')" wire:loading.attr="disabled" {{ $this->selectedCount ? '' : 'disabled' }}>
                    {{ __('Delete Selected') }}
                </button>
            @endcan

            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="CourseDetail" format="csv" />
                <livewire:excel-export model="CourseDetail" format="xlsx" />
                <livewire:excel-export model="CourseDetail" format="pdf" />
            @endif


            @can('course_detail_create')
                <x-csv-import route="{{ route('admin.course-details.csv.store') }}" />
            @endcan

        </div>
        <div class="w-full sm:w-1/2 sm:text-right">
            Search:
            <input type="text" wire:model.debounce.300ms="search" class="w-full sm:w-1/3 inline-block" />
        </div>
    </div>
    <div wire:loading.delay>
        Loading...
    </div>

    <div class="overflow-hidden">
        <div class="overflow-x-auto">
            <table class="table table-index w-full">
                <thead>
                    <tr>
                        <th class="w-9">
                        </th>
                        <th class="w-28">
                            {{ trans('cruds.courseDetail.fields.id') }}
                            @include('components.table.sort', ['field' => 'id'])
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.title') }}
                            @include('components.table.sort', ['field' => 'title'])
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.short_description') }}
                            @include('components.table.sort', ['field' => 'short_description'])
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.description') }}
                            @include('components.table.sort', ['field' => 'description'])
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.upload_image') }}
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.image_1') }}
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.instructor') }}
                            @include('components.table.sort', ['field' => 'instructor.title'])
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.category') }}
                            @include('components.table.sort', ['field' => 'category'])
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.duration') }}
                            @include('components.table.sort', ['field' => 'duration'])
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.batch') }}
                            @include('components.table.sort', ['field' => 'batch'])
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.lesson') }}
                            @include('components.table.sort', ['field' => 'lesson'])
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.student') }}
                            @include('components.table.sort', ['field' => 'student'])
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.skill_level') }}
                            @include('components.table.sort', ['field' => 'skill_level'])
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.language') }}
                            @include('components.table.sort', ['field' => 'language'])
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.price') }}
                            @include('components.table.sort', ['field' => 'price'])
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.languages') }}
                        </th>
                        <th>
                            {{ trans('cruds.courseDetail.fields.lesson_detail') }}
                        </th>
                        <th>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($courseDetails as $courseDetail)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $courseDetail->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $courseDetail->id }}
                            </td>
                            <td>
                                {{ $courseDetail->title }}
                            </td>
                            <td>
                                {{ $courseDetail->short_description }}
                            </td>
                            <td>
                                {{ $courseDetail->description }}
                            </td>
                            <td>
                                @foreach($courseDetail->upload_image as $key => $entry)
                                    <a class="link-photo" href="{{ $entry['url'] }}">
                                        <img src="{{ $entry['thumbnail'] }}" alt="{{ $entry['name'] }}" title="{{ $entry['name'] }}">
                                    </a>
                                @endforeach
                            </td>
                            <td>
                                @foreach($courseDetail->image_1 as $key => $entry)
                                    <a class="link-photo" href="{{ $entry['url'] }}">
                                        <img src="{{ $entry['thumbnail'] }}" alt="{{ $entry['name'] }}" title="{{ $entry['name'] }}">
                                    </a>
                                @endforeach
                            </td>
                            <td>
                                @if($courseDetail->instructor)
                                    <span class="badge badge-relationship">{{ $courseDetail->instructor->title ?? '' }}</span>
                                @endif
                            </td>
                            <td>
                                {{ $courseDetail->category }}
                            </td>
                            <td>
                                {{ $courseDetail->duration }}
                            </td>
                            <td>
                                {{ $courseDetail->batch }}
                            </td>
                            <td>
                                {{ $courseDetail->lesson }}
                            </td>
                            <td>
                                {{ $courseDetail->student }}
                            </td>
                            <td>
                                {{ $courseDetail->skill_level }}
                            </td>
                            <td>
                                {{ $courseDetail->language }}
                            </td>
                            <td>
                                {{ $courseDetail->price }}
                            </td>
                            <td>
                                @foreach($courseDetail->languages as $key => $entry)
                                    <span class="badge badge-relationship">{{ $entry->title }}</span>
                                @endforeach
                            </td>
                            <td>
                                @foreach($courseDetail->lessonDetail as $key => $entry)
                                    <span class="badge badge-relationship">{{ $entry->title }}</span>
                                @endforeach
                            </td>
                            <td>
                                <div class="flex justify-end">
                                    @can('course_detail_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.course-details.show', $courseDetail) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('course_detail_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.course-details.edit', $courseDetail) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('course_detail_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $courseDetail->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="10">No entries found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    </div>

    <div class="card-body">
        <div class="pt-3">
            @if($this->selectedCount)
                <p class="text-sm leading-5">
                    <span class="font-medium">
                        {{ $this->selectedCount }}
                    </span>
                    {{ __('Entries selected') }}
                </p>
            @endif
            {{ $courseDetails->links() }}
        </div>
    </div>
</div>

@push('scripts')
    <script>
        Livewire.on('confirm', e => {
    if (!confirm("{{ trans('global.areYouSure') }}")) {
        return
    }
@this[e.callback](...e.argv)
})
    </script>
@endpush